...
class opts(object):
  def __init__(self):
    ...
    # ddd
    self.parser.add_argument('--aug_ddd', type=float, default=0.0,
  ...
  def parse(self, args=''):
    ...
    opt.root_dir = "/mnt/nfs/zzwu/04_centerNet/CenterNet-master_ph"
    data_root_dir = "/mnt/nfs/zzwu/04_centerNet/CenterNet-master_lyc"
    #print("opt.root_dir:",opt.root_dir)
    opt.data_dir = os.path.join(data_root_dir, 'data')
    #print("opt.data_dir:",opt.data_dir)
    opt.exp_dir = os.path.join(opt.root_dir, 'exp', opt.task)
    #opt.save_dir = os.path.join(opt.exp_dir, opt.exp_id)
    opt.save_dir = "/mnt/nfs/zzwu/04_centerNet/CenterNet-master_ph/exp/ddd"
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      opt.heads = {'hm': opt.num_classes, 'dep': 1, 'rot': 6, 'dim': 3}
      if opt.reg_bbox:
        opt.heads.update(
          {'wh': 2})
      if opt.reg_offset:
