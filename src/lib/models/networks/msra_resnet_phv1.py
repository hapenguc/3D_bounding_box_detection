...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv, **kwargs):
        ...
        self.reducelayer = self.dddector = nn.Sequential(
                  nn.Conv2d(512, 256,
                    kernel_size=3, padding=1, bias=True),
                  nn.ReLU(inplace=True))       
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels):
        ...
        for i in range(num_layers):
            ...
            layers.append(
                nn.ConvTranspose2d(
                    in_channels=self.inplanes,   #64,256,256
                    out_channels=planes,        #256 256 256
    ...
    def forward(self, x):
        ...
        x_1 = torch.cat((hm,wh,reg), 1)
        #print("x_:",x_.shape) #[4,7,12,40]
        
        x_1 = self.dddector(x_1)  #[4,256,12,40]
        x = torch.cat((x,x_1),1)
        print("x:",x.shape)   #[4,512,12,40]
        
        x = self.reducelayer(x)  #[4,256,12,40]
...
def get_pose_net_phv1(num_layers, heads, head_conv):
