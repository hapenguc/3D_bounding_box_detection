...
class PoseResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("x:",x.shape) [4,256,96,320]
        hm = self.hm_layer(x)
        wh = self.wh_layer(x)
        reg = self.reg_layer(x)
        
        #print("hm",hm.shape) [4,3,96,320]
        x_1 = torch.cat((hm,wh,reg), 1)
        #print("x_:",x_.shape) #[4,7,96,320]
        
        x_1 = self.dddector(x_1)
        x = torch.cat((x,x_1),1)
        print("x:",x.shape)
...
def get_pose_net_v3(num_layers, heads, head_conv):
