...
#post process give the 3dlocation-version dets
def get_pred_depth(depth):
  return depth

def get_alpha(rot):
  # output: (B, 6) [, bin1_cls[1], bin1_sin, bin1_cos, 
  #                  bin2_cls[1], bin2_sin, bin2_cos]
  # return rot[:, 0]
  idx = rot[:, 0] > rot[:, 3]
  print('******************************************')
  print(rot[:, 0] > rot[:, 3])
  print('*************************************')
  alpha1 = np.arctan(rot[:, 1] / rot[:, 2]) + (-0.5 * np.pi)
  alpha2 = np.arctan(rot[:, 4] / rot[:, 5]) + ( 0.5 * np.pi)
  ...
  for i in range(dets.shape[0]):
    ...
    for j in range(opt.num_classes):
      ...
      top_preds[j + 1] = np.concatenate([
        ...
        get_alpha(dets[i, inds, 3:9])[:, np.newaxis].astype(np.float32),
        get_pred_depth(dets[i, inds, 9:10]).astype(np.float32),
        dets[i, inds, 10:13].astype(np.float32)], axis=1)
      if include_wh:
        top_preds[j + 1] = np.concatenate([
          top_preds[j + 1],
          transform_preds(
            dets[i, inds, 13:15], c[i], s[i], (opt.output_w, opt.output_h))
...
def ddd_post_process_3d(dets, calibs):
  ...
  for i in range(len(dets)):
    ...
    for cls_ind in dets[i].keys():
      ...
      for j in range(len(dets[i][cls_ind])):
        ...
        locations, rotation_y = ddd2locrot(
          center, alpha, dimensions, depth, calibs[0])   ###########################here gives the 3D center coordinate
    ...
    ret.append(preds)
  return ret        # there is no depth but location(3)
