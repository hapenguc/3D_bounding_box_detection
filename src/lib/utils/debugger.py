...
class Debugger(object):
  ...
  def save_all_imgs(self, path='', prefix='', genID=False):
    ...
    #print("path:",path)
    for i, v in self.imgs.items():
      cv2.imwrite(path + '/{}{}'.format(i,image_id), v)
  ...
  def project_3d_to_bird(self, pt):
    #print("oldsize pt[0]:",pt[0])
    #print("oldsize pt[1]:",pt[1])
    pt[0] += self.world_size / 2
    pt[1] = self.world_size - pt[1]
    pt = pt * self.out_size / self.world_size
    #print("newsize pt[0]:",pt[0])
    #print("newsize pt[1]:",pt[1])
  ...
  def add_3d_detection(
    ...
    center_thresh=0.5, img_id='det'):
    #print("image_or_path:",image_or_path)
    ...
    for cat in dets:
      for i in range(len(dets[cat])):
        ...
        if dets[cat][i, -1] > center_thresh:
          ...
          if loc[2] > 1:
            box_3d = compute_box_3d(dim, loc, rot_y)        ############   these function comes from ddd_utils
            box_2d = project_to_image(box_3d, calib)                                        ###################################################### add 3d boxes
  ...
  def add_bird_view(self, dets, center_thresh=0.3, img_id='bird'):
    ...
    for i in range(15):  
      if i == 7:
        cv2.line(bird_view,(24*(i+1),0),(24*(i+1),383),(0,0,255),3)
      else:
        cv2.line(bird_view,(24*(i+1),0),(24*(i+1),383),(0,255,0),3)

    for i in range(15):  
        cv2.line(bird_view,(0,24*(i+1)),(383,24*(i+1)),(0,255,0),3)
