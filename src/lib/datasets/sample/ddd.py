...
class DddDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    #print("inp:",inp.shape)  #(384,1280,3)
    ...
    for k in range(num_objs):
      ...
      if h > 0 and w > 0:
        ...
        if 1:
          ...
          #print("reg_mask[k]:",reg_mask[k])
          rot_mask[k] = 1
    #print('gt_det', gt_det)
    # print('')
    # print("rot_mask:",rot_mask)
  ...
  def _alpha_to_8(self, alpha):
    ...
    if alpha < np.pi / 6. or alpha > 5 * np.pi / 6.:
      r = alpha - (-0.5 * np.pi)
      ret[0] = 1
      ret[1], ret[2] = np.sin(r), np.cos(r)
    if alpha > -np.pi / 6. or alpha < -5 * np.pi / 6.:
      r = alpha - (0.5 * np.pi)
      ret[3] = 1
      ret[4], ret[5] = np.sin(r), np.cos(r)
    return ret
